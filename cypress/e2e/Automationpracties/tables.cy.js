...
describe ('how to handle tables', () => {
    it ('tables', () => {
        cy.visit('https://testautomationpractice.blogspot.com/')
        // text
        cy.xpath('//table[@name="BookTable"]/tbody/tr[2]/td[4]').then((txt) => {
            var trtxt = txt.text()
            expect(trtxt).to.equal('300')
        })
        // how many rows are their
        cy.xpath('//table[@name="BookTable"]/tbody/tr').its(length).then((len) => {
            var rowlength = len 
            cy.log(rowlength)
        })
        cy.xpath('//table[@name="BookTable"]/tbody/tr').should('have.length',7)
        // how many colums are their

        cy.xpath('//table[@name="BookTable"]/tbody/tr/th').should('have.length',4)
